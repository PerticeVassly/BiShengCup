...
float_absEntry:
  ...
  br i1 %cond_, label %ifTrue_262, label %next_458

ifTrue_262:                                            ; pred = %float_absEntry
  %x$1 = load float, float* %lv, align 4
  %tmp_ = fsub float 0x0, %x$1
  ret float %tmp_

next_458:                                              ; pred = %float_absEntry
...
float_eqEntry:
  ...
  %cond_tmp_ = zext i1 %cond_lt_tmp_ to i32
  %cond_ = icmp ne i32 %cond_tmp_, 0
  br i1 %cond_, label %ifTrue_263, label %ifFalse_112

ifTrue_263:                                           ; pred = %float_eqEntry
  ret i32 1

ifFalse_112:                                          ; pred = %float_eqEntry
  ret i32 0
}
...
assertEntry:
  ...
  br i1 %cond_normalize_, label %ifTrue_264, label %ifFalse_113

ifTrue_264:                                         ; pred = %assertEntry
  call void @error()
  br label %next_460

ifFalse_113:                                        ; pred = %assertEntry
  call void @ok()
  br label %next_460

next_460:                                           ; pred = %ifTrue_264, %ifFalse_113
  ret void
}
...
assert_notEntry:
  ...
  br i1 %cond_normalize_, label %ifTrue_265, label %ifFalse_114

ifTrue_265:                                             ; pred = %assert_notEntry
  call void @error()
  br label %next_461

ifFalse_114:                                            ; pred = %assert_notEntry
  call void @ok()
  br label %next_461

next_461:                                               ; pred = %ifTrue_265, %ifFalse_114
  ret void
}
...
mainEntry47:
  ...
  br i1 %cond_normalize_, label %ifTrue_266, label %next_462

ifTrue_266:                                           ; pred = %mainEntry47
  call void @ok()
  br label %next_462

next_462:                                             ; pred = %mainEntry47, %ifTrue_266
  %cond_normalize_$1 = icmp ne i32 1, 0
  br i1 %cond_normalize_$1, label %ifTrue_267, label %next_463

ifTrue_267:                                           ; pred = %next_462
  call void @ok()
  br label %next_463

next_463:                                             ; pred = %next_462, %ifTrue_267
  %cond_normalize_$2 = fcmp one float 0x0, 0x0
  br i1 %cond_normalize_$2, label %secondCond_97, label %next_464

ifTrue_268:                                           ; pred = %secondCond_97
  call void @error()
  br label %next_464

next_464:                                             ; pred = %next_463, %secondCond_97, %ifTrue_268
  %cond_normalize_$4 = icmp ne i32 0, 0
  br i1 %cond_normalize_$4, label %ifTrue_269, label %secondCond_98

secondCond_97:                                        ; pred = %next_463
  %cond_normalize_$3 = icmp ne i32 3, 0
  br i1 %cond_normalize_$3, label %ifTrue_268, label %next_464

ifTrue_269:                                           ; pred = %next_464, %secondCond_98
  call void @ok()
  br label %next_465

next_465:                                             ; pred = %secondCond_98, %ifTrue_269
...
secondCond_98:                                        ; pred = %next_464
  %cond_normalize_$5 = fcmp one float 0x3fd3333340000000, 0x0
  br i1 %cond_normalize_$5, label %ifTrue_269, label %next_465

whileCond_196:                                        ; pred = %next_465, %whileBody_196
  %i = load i32, i32* %lv, align 4
  %cond_lt_tmp_ = icmp slt i32 %i, 1000000000
  %cond_tmp_ = zext i1 %cond_lt_tmp_ to i32
  %cond_ = icmp ne i32 %cond_tmp_, 0
  br i1 %cond_, label %whileBody_196, label %next_466
